...
TOKEN arrayref(TOKEN arr, TOKEN tok, TOKEN subs, TOKEN tokb) {
  ...
  TOKEN out;
  if (subs->link) {
    s = searchst(arr->stringval);
    int outerelemsize = s->datatype->datatype->size;
    int innerelemsize = s->datatype->datatype->datatype->size;
    tok = makeop(TIMESOP);
    tokb = makeop(PLUSOP);
    TOKEN outertok = makeintc(outerelemsize);
    outertok->link = subs;
    TOKEN next = copytok(subs->link);
    subs->link = NULL;
    TOKEN neg = makeintc((-1 * outerelemsize) + (next->intval * innerelemsize));
    neg->link = tok;
    tokb->operands = neg;
    tok->operands = outertok;
    out = makearef(arr, tokb ,talloc());
    return out;
  } else {
    s = searchst(arr->stringval);
    int elemsize = s->datatype->datatype->size;
    if ((s = searchst(subs->stringval)) != NULL) {
      tok = makeop(TIMESOP);
      tokb = makeop(PLUSOP);
      TOKEN neg = makeintc(-elemsize);
      ...
      out = makearef(arr, makeintc(elemsize * (subs->intval - 1)) ,tokb);
      return out;
    }
  }
}
